...
if path.exists('daily_data.txt') == False:
    ...
    with open('daily_data.txt') as json_file:
        ...
        for key in data_dict:
            ...
            daily_data[key]['KD'][current_day] = kill_count/death_count
            daily_data[key]['winrate'][current_day] = win_count/game_count
            
with open('daily_data.txt', 'w+', encoding='utf-8') as f:
     json.dump(daily_data, f, ensure_ascii=False, indent=3)
if path.exists("StatBotInfo.txt") == True:
    source = "StatBotInfo.txt"
    destination = "DailyDataStorage"
    new_path = shutil.move(source,destination)
    Current_date = datetime.datetime.today().strftime ('%d-%b-%Y')
    os.rename(r'C:\Users\dargi\Desktop\Coding\ValorantBot\DailyDataStorage\StatBotInfo.txt',r'C:\Users\dargi\Desktop\Coding\ValorantBot\DailyDataStorage\StatBotInfo_day' + str(current_day)+ '_' + str(Current_date) + '.txt')
